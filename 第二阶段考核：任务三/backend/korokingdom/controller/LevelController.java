...
public class LevelController {
    ...
    public Map<String, Object> claimReward(@RequestHeader("Authorization") String authorization,
        ...
        // 6.5 随机发放药剂（50%概率得HP药，50%概率得PP药）
        Random random = new Random();
        String potionType = null;
        if (random.nextBoolean()) {
            int newCount = (pet.getHpPotionCount() == null ? 0 : pet.getHpPotionCount()) + 1;
            pet.setHpPotionCount(newCount);
            potionType = "HP药剂";
        } else {
            int newCount = (pet.getPpPotionCount() == null ? 0 : pet.getPpPotionCount()) + 1;
            pet.setPpPotionCount(newCount);
            potionType = "PP药剂";
        }
